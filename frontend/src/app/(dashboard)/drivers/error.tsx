...
export default function DriversError({
  ...
  useEffect(() => {
    console.error('Drivers route error:', error);
  }, [error]);
  ...
  return (
    <PageContainer className="flex items-center justify-center min-h-[70vh] select-none text-left">
      <div className="text-center py-12 px-6 border border-border/60 bg-card rounded-2xl max-w-md shadow-sm">
        ...
        <h2 className="text-lg font-extrabold text-foreground tracking-tight">Unable to Load Drivers</h2>
        <p className="text-xs text-muted-foreground mt-2 leading-relaxed">
          An error occurred while loading the driver roster directory. Please try reloading the workspace state.
        </p>
